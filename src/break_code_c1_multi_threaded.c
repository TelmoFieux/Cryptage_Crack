...
list_t** crack_C1(const char* crypted_mess_filename, int keysize){
    ...
    for (int i = 0; i<63; i++){
        ...
        for (int j = 0; j<keysize; j++){
            ...
            int elem_red;
            while ((elem_red=fread(&reader,sizeof(unsigned char),sizeof(reader),crypted_mess_file))!=0) {
                int index=0;
                while (index*keysize <elem_red){
                    if (!isalnum(test_car^reader[index*keysize]) && !ispunct(test_car^reader[index*keysize]) && !isspace(test_car^reader[index*keysize])){
                        valid = false;
                        break;
                    }
                    index++;
